...
export default function FoodCard({allListedFood,myDonation}) {
  ...
  return (
    <div>
          ...
          <Card shadow="sm" key={index} className="cursor-pointer">
            ...
            <CardFooter className="text-sm w-full flex flex-col">
              ...
              { !myDonation? <Button
                ...
                >
                 {item.distributed ? "Distributed": " Mark As Distributed"}
                  <i className="fa-solid fa-hand-holding-medical"></i>
                </Button>}
              </div>
            </CardFooter>
      ...
      <DistrbuteConfirm id={selectedFoodItem._id} isDistrubuteDone={isDistrubuteDone} setIsDistrubteDone={setIsDistrubteDone}></DistrbuteConfirm>
    </div>
    </div>
  );
}
